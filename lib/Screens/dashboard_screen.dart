...
class _DashBoardScreeenState extends State<DashBoardScreeen> {
  ...
  Future<void> _selectedDate(BuildContext context,
    ...
    DateTime initialDate =
        DateTime.now(); // Default to current date for initial selection
    ...
    if (!isFromDate) {
      ...
      initialDate =
          _fromDate!; // Default initial date is the selected "From Date"
    }

    // Set the first date for the date picker based on whether it's "From Date" or "To Date"
    DateTime firstDate = isFromDate
        ? DateTime.now()
        : _fromDate!; // Allow "To Date" from the selected "From Date" onwards
  ...
  Future<void> applyTicketRequest(String departureText, String arrivalText,
    ...
    final box = GetStorage();
    String empId =
        box.read('userId') ?? ''; // Use the value from the userIdController
    print(empId);
  ...
  Future<void> fetchTicketRequests() async {
    try {
      ...
      final request = ModelQueries.list(
        ...
        where:
            TicketRequest.EMPID.eq(empId), // Fetch tickets for the current user
      );
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            MaterialButton(
                              minWidth:
                                  size.width * 0.060, // Adjust width as needed
                                  ...
                                  showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        ...
                                        content: Text(
                                            'Are you sure you want to apply for the ticket?'),
                                        actions: [
                                          TextButton(
                                            onPressed: () {
  ...
  Future<void> fetchLeaveData() async {
    try {
      ...
      final request = ModelQueries.list<LeaveStatus>(
          LeaveStatus.classType); // Explicitly specify the type
      final response = await Amplify.API.query(request: request).response;
      print(response);
      ...
      List<LeaveStatus?> userLeaveStatuses =
          leaveStatuses.where((leave) => leave?.empID == empId).toList();
  ...
  Future<void> deleteLeave(String empId) async {
    try {
      ...
      if (response.errors.isNotEmpty) {
        ...
        _showAlertDialog(
            'Error', 'Failed to fetch leave records for deletion.');
        return;
      }

      // Get all leave records for the specified empId
      List<LeaveStatus?> leaveRecords = response.data?.items ?? [];
      List<LeaveStatus?> leavesToDelete =
          leaveRecords.where((leave) => leave?.empID == empId).toList();

      if (leavesToDelete.isEmpty) {
        print('No leave records found for empId: $empId');
        _showAlertDialog(
            'Error', 'No leave records found for this employee ID.');
        return;
      }
      ...
      for (LeaveStatus? leave in leavesToDelete) {
        if (leave != null) {
          ...
          if (deleteResponse.errors.isNotEmpty) {
            print(
                'Errors deleting leave ID ${leave.id}: ${deleteResponse.errors}');
  ...
  Future<void> updateTicketStatusById(String ticketId) async {
    try {
      ...
      final getRequest = ModelQueries.get(
          TicketRequest.classType, ticketId as ModelIdentifier<TicketRequest>);
  ...
  Future<void> updateLeaveStatus(LeaveStatus leave) async {
    // Create a new copy of the leave with the updated empStatus field
    final updatedLeave =
        leave.copyWith(empStatus: 'Cancelled', applyTo: leave.applyTo);
...
// Success dialog after cancellation
  ...
  void _showphoneLeaveDialog(
      BuildContext context, int index, LeaveStatus leave) {
    // Show the appropriate dialog based on leave status
    if (leave.empStatus == 'Approved') {
  ...
  void _showphoneTicketDialog(
      BuildContext context, int index, TicketRequest request) {
    // Show the appropriate dialog based on leave status
                    ...
                    return DataRow(
                      cells: [
                        DataCell(
                          ...
                          onTap: () {
                            _showLeaveDialog(
                                context, index, leave); // Method to handle tap
                          },
                        ),
                        DataCell(
                          Text(
                        ...
                        DataCell(
                          Text(leave?.empStatus?.toString() ?? 'Approved',
                              style: rowTextStyle),
                          onTap: () {
                            _showLeaveDialog(context, index, leave);
                          },
                ...
                child: DataTable(
                  ...
                  rows: filteredTicketRequests.map((request) {
                    ...
                    return DataRow(cells: [
                      DataCell(Text('Rahul', style: rowTextStyle), onTap: () {
                        _showTicketDialog(context, index, request!);
                      }), // Static Name
                      DataCell(Text('50598', style: rowTextStyle), onTap: () {
                        _showTicketDialog(context, index, request!);
                      }), // Static Badge Number
                      DataCell(Text('Welding', style: rowTextStyle), onTap: () {
                        _showTicketDialog(context, index, request!);
                      }), // Static Department
                      DataCell(Text('Trainer', style: rowTextStyle), onTap: () {
                        _showTicketDialog(context, index, request!);
                      }), // Static Position
                      DataCell(
                          Text(request?.destination ?? 'Unknown',
                              style: rowTextStyle), onTap: () {
                        _showTicketDialog(context, index, request!);
                      }), // Dynamic Destination
                      DataCell(
                          Text(
                      ...
                      DataCell(
                          Text(
                              request!.arrivalDate != null
                                  ? DateFormat('dd/MM/yyyy').format(
                                      request.arrivalDate!.getDateTime())
                                  : 'N/A',
                              style: rowTextStyle), onTap: () {
                        _showTicketDialog(context, index, request!);
                      }),
                      ...
                      DataCell(
                          Text(
                            request?.hrStatus ??
                                'Rejected', // Show status text (default to 'Pending' if null)
                            style: rowTextStyle,
                          ), onTap: () {
                        _showTicketDialog(context, index, request!);
                      })
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                ...
                columns: [
                  ...
                  DataColumn(
                      label: Text('Badge Number', style: phoneheaderTextStyle)),
                  DataColumn(
                      label: Text('Department', style: phoneheaderTextStyle)),
                  DataColumn(
                      label: Text('Position', style: phoneheaderTextStyle)),
                  DataColumn(
                      label: Text('Destination', style: phoneheaderTextStyle)),
                  DataColumn(
                      label:
                          Text('Departure Date', style: phoneheaderTextStyle)),
                  DataColumn(
                      label: Text('Arrival Date', style: phoneheaderTextStyle)),
                  DataColumn(
                      label: Text('Status', style: phoneheaderTextStyle)),
                  DataColumn(
                      label: Text('Remarks', style: phoneheaderTextStyle)),
                ],
                rows: [
                  DataRow(cells: [
                    DataCell(Text(
